...
describe('uploadRunLog — flow_run.logsFileId FK safety', () => {
    it('omits logsFileId when a worker internal error is not persisted to a log file (Cloud)', async () => {
        ...
        // Cloud + worker-source internal error → persistInternalErrorToLogs is skipped, so no file row exists.
        // The dangling logsFileId must NOT be written to flow_run, otherwise fk_flow_run_logs_file_id throws.
